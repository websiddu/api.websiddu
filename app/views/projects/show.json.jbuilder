...
json.nextProject Project.where({:sequence => {:$eq => @project.sequence+1}}).sort({:_id=>1}).first
json.previousProject Project.where({:sequence => {:$eq => @project.sequence-1}}).sort({:_id=>1}).first
